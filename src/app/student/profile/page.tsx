...
import { useToggleNameVisibility } from "@/hooks/use-profile";
...
export default function ProfilePage() {
  ...
  const toggleNameVisibility = useToggleNameVisibility();
  ...
  useEffect(() => {
    const fetchScores = async () => {
      try {
        ...
        setScoreError(
          err?.response?.data?.message || "Failed to load exam scores",
        );
      } finally {
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      <main className="flex-1 w-full mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 max-w-7xl">
        ...
        <Card className="mb-6 py-0 sm:mb-8 lg:mb-10 overflow-hidden shadow-md">
          ...
          <CardContent className="p-4 sm:p-6 lg:p-8">
            <div className="flex items-center justify-between gap-4 rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 mb-4 sm:mb-6">
              <div>
                <p className="text-sm font-medium text-gray-800">
                  Show my name in Scoreboard
                </p>
                <p className="text-xs text-gray-500 mt-0.5">
                  When off, your rank appears anonymously to other students
                </p>
              </div>
              <Switch
                checked={!student?.is_hidden}
                disabled={toggleNameVisibility.isPending}
                onCheckedChange={() => toggleNameVisibility.mutate()}
                aria-label="Toggle name visibility in scoreboard"
          ...
          {scoreLoading ? (
            ...
            <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
              <p className="text-gray-500 text-sm sm:text-base">
                No exam results found. Complete your first exam to see results
                here.
              </p>
            </div>
          ) : (
