...
export default function StudentDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"all" | "solved" | "unsolved">(
    "all",
  );
  ...
  const handleError = (error: any, type: ErrorState["type"]) => {
    const errorMessage =
      error?.data?.errors || error?.message || "An unexpected error occurred";
  ...
  const handleEditAction = async (id: number, updatedQuestion: string) => {
    try {
      clearError();
      const response = await forumService.editOwnForumQuestion(id, {
  ...
  const EmptyState = ({ type }: { type: "all" | "solved" | "unsolved" }) => {
    const config = {
      ...
      solved: {
        ...
        message:
          "Your solved questions will appear here once they're answered.",
    ...
    <div className="w-full max-w-7xl px-4 mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div>
            <h3 className="text-xl sm:text-2xl font-semibold font-poppins text-black mb-3">
              Subject
            </h3>
            {subjectsLoading ? (
          ...
          >
            <h3 className="text-lg sm:text-xl font-semibold font-poppins text-black">
              Got a question? Drop it here!
            </h3>

            <AnimatePresence>
              {errorState.hasError && errorState.type === "submit" && (
                <ErrorAlert />
              )}
            </AnimatePresence>
          ...
          <div className="space-y-4">
            ...
            <AnimatePresence>
              {errorState.hasError && errorState.type === "fetch" && (
                <ErrorAlert />
              )}
            </AnimatePresence>
                  ...
                  {filteredQuestions.map((question, index) => (
                    ...
                    >
                      <RepliesCard
                        ...
                        onBlockAction={() => handleAllQuestions(currentPage)}
                      />
                    </motion.div>
                  ))}
                </motion.div>
        ...
        <div className="space-y-6">
          ...
          >
            <h2 className="text-lg sm:text-xl lg:text-2xl font-semibold font-poppins text-black mb-4">
              Overall Performance
            </h2>
